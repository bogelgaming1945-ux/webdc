...
-- Button to send all player IDs to the configured webhook
local SendIDsButton = Tab:CreateButton({
   Name = "Send All Player IDs",
   Callback = function()
      ...
      -- Build lines and chunk if message too long for Discord (safety margin)
      local lines = {}
      ...
      for _, chunk in ipairs(chunks) do
         local data = { content = "**Player IDs ("..#ids..")**\n".."```\n"..chunk.."```" }
      ...
      if successAll then
         Rayfield:Notify({Title = "Success", Content = "Player IDs sent to webhook!", Duration = 4})
      else
